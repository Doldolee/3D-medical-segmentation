...
DATA_DIR_SPLEEN = "./data/Task09_Spleen"
...
def get_dataset(data_type, image_size, batch_size):
    ...
    if data_type == 'decathron_spleen':
        ...
        img_path_list = sorted(glob.glob(os.path.join(DATA_DIR_SPLEEN, "imagesTr", "*.nii.gz")))
        mask_path_list = sorted(glob.glob(os.path.join(DATA_DIR_SPLEEN, "labelsTr", "*.nii.gz")))
